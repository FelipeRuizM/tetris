...
public:
	// STATIC CONSTANTS
	static const int CHANGE_LEVEL_EVERY_ROW = 10;
	static const int BLOCK_WIDTH  = 32;			  // pixel width of a tetris block, init to 32
	static const int BLOCK_HEIGHT = 32;			  // pixel height of a tetris block, int to 32
	static const int LEVEL_QUANTITY = 6;
	static constexpr double MAX_SECONDS_PER_TICK = 0.75; // the slowest "tick" rate (in seconds), init to 0.75
	static constexpr double MIN_SECONDS_PER_TICK = 0.2; // the fastest "tick" rate (in seconds), init to 0.20
...
private:	
	...
	std::string scores = "scores/scores.txt";
	...
	double decreaseTickBy = (MAX_SECONDS_PER_TICK - MIN_SECONDS_PER_TICK) / LEVEL_QUANTITY;
...
private:
	...
	int getScore(int completedRowsNum) const;
